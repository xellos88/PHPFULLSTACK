...
echo "*******************************************\n";
echo "딜러: {$dealer_hand[0]}" . "\n";
echo "플레이어: " . implode(", ", $player_hand) . " ( 합계: " . value_hand($player_hand) . ")" . "\n";
echo "********************************************\n";

// 플레이어
while (value_hand($player_hand) < 21) 
{
    echo "카드를 더 받으시겠습니까? (예 → 1 / 아니오 → 2 / 종료 → 0) ";
    $choice = readline();
    if ($choice === '1') 
    {
        $player_hand[] = array_pop($deck);
...
// 딜러
if (value_hand($player_hand) > 21) 
{
    echo "플레이어가 버스트되었습니다." . "\n";
} 
else 
{
    echo "딜러: " . implode(", ", $dealer_hand) . " ( 합계: " . value_hand($dealer_hand) . ")" . "\n";
    while (value_hand($dealer_hand) < 17) 
    {
        $dealer_hand[] = array_pop($deck);
        echo "딜러: " . implode(", ", $dealer_hand) . " ( 합계: " . value_hand($dealer_hand) . ")" . "\n";
    }
    if (value_hand($dealer_hand) > 21) 
    {
        echo "딜러가 버스트되었습니다." . "\n";
    } else 
    {
        if (value_hand($player_hand) > value_hand($dealer_hand)) 
        {
            echo "플레이어 승리!" . "\n";
        } else if (value_hand($player_hand) < value_hand($dealer_hand)) 
        {
            echo "딜러 승리!" . "\n";
        } else 
        {
